...
imgs=$(curl -ks 'https://console.cloud.google.com/m/gcr/entities/list' \
    ...
    --data-binary '["google-containers"]' | grep -P '"' | sed 's/"gcr.ListEntities"//' |cut -d '"' -f2 |sort |uniq)
...
for img in ${imgs[@]}; do
    ...
    echo ${gcr_content} | jq -r '.manifest[]|{k: .tag[0],v: .timeUploadedMs} | "touch -amd \"$(date -d @" + .v[0:10] +")\" gcr.io_mirror\/google_containers\/${img}\/"  +.k' | while read i;
    do
        eval $i
    done
